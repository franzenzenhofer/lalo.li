...
(function() {
  var INTRO_STRING, checksum, ifSpeakErrorShowError, makeSpeakHash, saySo, short_url_visible, showError, toHash;
  ...
  $(function() {
    ...
    return $(window).load(function() {
      ...
      $('#text').on('webkitspeechchange', (function(e) {
        e.stopPropagation();
        makeSpeakHash();
        return saySo();
      }));
      $('body').on('speechchange', (function(e) {
        e.stopPropagation();
        makeSpeakHash();
        return saySo();
      }));
      ...
      $("#twitter").on('click', function() {
        return this.href = 'http://twitter.com/intent/tweet?text=' + encodeURIComponent('Voice Message') + '&url=http://www.lalo.li/?' + encodeURIComponent(window.location.hash.slice(1) || window.location.search.slice(1));
      });
      $("#gplus").on('click', function() {
        return this.href = 'https://plus.google.com/share?url=http://www.lalo.li/?' + encodeURIComponent(window.location.hash.slice(1) || window.location.search.slice(1));
      });
      $("#facebook").on('click', function() {
        return this.href = 'https://www.facebook.com/sharer.php?u=http://www.lalo.li/?' + encodeURIComponent(window.location.hash.slice(1) || window.location.search.slice(1)) + '&t=' + encodeURIComponent('Voice Message');
      });
      return $('#bitly').on('click', function() {
        $.getJSON('http://api.bit.ly/v3/shorten?login=laloli&&apiKey=R_f29b99469604b70e8c5898e05ec09daa&longUrl=http://www.lalo.li/?' + encodeURIComponent(window.location.hash.slice(1) || window.location.search.slice(1)) + '&format=json&callback=?', (function(response) {
          var _ref;
          if (response != null ? (_ref = response.data) != null ? _ref.url : void 0 : void 0) {
            short_url_visible = true;
            return $('#shorturl').fadeOut().fadeIn().html('Short-URL: <a target="_blank" href="' + response.data.url + '">' + response.data.url + '</a>');
          }
        }));
        return false;
      });
